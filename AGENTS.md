...
├── engine/intent.py     Window title -> category. What you were doing inside
│                         an app, when the app alone is too coarse to say.
...
### Categorisation: two layers (2026-08-20)
`engine/apps.py` answers "what is this app" and `engine/titles.py` answers "what were you doing in it". The second exists because the first cannot: a browser is a documentation reader and a television, often in the same minute, and `opera.exe -> mixed` is wrong both times. A title rule matches a case-insensitive substring of the window title and **overrides** the app's category for exactly the spans where it matched; when nothing matches, the app's own category stands, which is byte-for-byte the previous behaviour. That fallback is what makes it safe: a capture made with `--no-titles` degrades to app-only categorisation instead of misclassifying anything.

Titles are read by `engine/db.py:read_events_titled()` and **never leave the engine**. `dashboard.json` gets rule patterns and totals, never a title — that file is imported straight into the browser bundle, and a window title carries the page you were reading. `read_events()` still drops titles entirely, so every caller that doesn't classify by title never sees the column. Two functions rather than one flag, deliberately.

**Input density** (`Block.key_count`, `keys_per_min`, and the `kpm` column in `engine.apps list`) is the other half. It separates production from consumption — 40 minutes of browser at 90 keys/min is not 40 minutes of browser at 2. It does **not** separate productive from unproductive: reading documentation carefully is low-input and valuable, and no timing signal distinguishes that from scrolling. Nothing in `engine/` ranks, scores or penalises a block on density, and nothing should. It is displayed next to the time and left to the reader.
...
| Threshold sliders read precomputed engine sweeps rather than recomputing in TypeScript | A TS reimplementation of Core/Fragments would be a second source of truth that could silently disagree with Python. Sweeping in `engine/export.py` keeps one implementation; the cost is discrete slider steps |
| **Correction (2026-08-10):** the row above originally claimed "§8 requires thresholds be tunable in the UI". It does not. §8 says thresholds are "visible in the UI and tunable in **config**" — a much weaker requirement that the inline "runs over 25 min" labels already satisfy. The Tuning panel was built past the spec, largely because it was the fastest way to investigate the unvalidated `IDLE_GAP` value. It is a development tool, not a shipped feature | Recording this because the misquote had already been written into the decision log as if it were a requirement, which is how an unexamined choice becomes a load-bearing one |
...
| Real dashboard exports gitignored; only `dashboard.sample.json` is tracked (2026-08-10) | `app/_data/dashboard.json` was committed, and the documented `npm run data:real` overwrites it with every app used and the clock time of every block worked. `.gitignore` covered `*.db` so the raw capture was safe while the derived file, which leaks nearly as much, was not. Caught by running a real export and watching it appear in `git status` |
| `engine/watch.py` re-exports on a timer rather than the collector doing it | The dashboard's JSON import hot-reloads on change, so a timed re-export is all "live updating" needs. It lives in `engine/` because `collector/` is the AGPL audited component and `engine/` is closed — a collector that imports the engine would drag closed code into the part users are meant to audit |
...
| Window titles classify intent, overriding the app's category, and no rule means no override (2026-08-20) | An app is the wrong unit: the same browser is Stack Overflow and YouTube within one minute, so any single tag for it is wrong half the time. Override-with-fallback rather than replace means a `--no-titles` capture, or a title nobody wrote a rule for, behaves exactly as it did before the feature existed — the failure mode is "no better than yesterday", never "confidently wrong" |
| Titles are read by the engine but never written to `dashboard.json` (2026-08-20) | The snapshot is imported into the browser bundle and has already been the wrong place for personal data once (the real-export leak, 2026-08-10). Rule patterns and totals answer "why did this number move" without putting the page you were reading into a build artifact |
| Input density (`keys_per_min`) is displayed, never scored (2026-08-20) | It genuinely separates typing in an app from watching one, which is what makes a browser's time legible. It cannot separate reading documentation from scrolling a feed, and a tool that ranks on it would be guessing and calling the guess a measurement — which is the exact failure that makes people distrust activity trackers |
| Threshold sweeps rebuild with the same `intent_of` as the dashboard (2026-08-20) | Caught by mypy while wiring it: the sweeps recompute blocks from raw events, so omitting intent would have made the Tuning panel disagree with the dashboard beside it, for reasons invisible to anyone reading either |
